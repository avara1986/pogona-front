...
(function(scope){
  ...
  gulp.task('p-scripts:package:now', function(){
    var jsBundles = [distDir + '/scripts/pogona.min.js'];
  ...
  var scriptsBrowserify = function(env){
    var jsBundles = ['app/scripts/pogona.js'];
...
(function(scope){
  ...
  gulp.task('vulcanize:inline:now',function(){
    return gulp.src(distDir + '/pogona.html', {base: distDir + '/'})
      .pipe($.vulcanize({
  ...
  var vulcanizeWithExternalScriptsAndStyles = function(vulcanizeOptions){
    return gulp.src(tmpDir + '/pogona.html')
